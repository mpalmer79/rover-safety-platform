...
* an evidence artifact produced when verification runs.
...
| `partial` | Some checks held; some are not yet evaluable from the available artifacts. The verifier produces a partial result rather than a fake pass. |
...
artifacts produced by either the deterministic engine or the ROS 2
launch.
...
| [`qualification_scenarios`](../backend/app/runtime_validation/qualification_scenarios.py) | Phase 5. YAML pack format + loader for qualification scenarios (one YAML per scenario; required topics / nodes / events / replay artifacts; expected outcome). | (consumed by the orchestrator) |
| [`baselines`](../backend/app/runtime_validation/baselines.py) | Phase 5. Per-category baseline + comparator (topics, nodes, TF, transitions, commands, events, replay artifacts, diagnostic health). Classifies deltas as `expected_difference`, `warning`, `regression`, or `critical_regression`. | `rover_ws/tools/compare_runtime_baseline.py` |
...
| [`replay_analytics`](../backend/app/replay_analytics/) | Phase 8. Read-only analytics over Phase-6 + Phase-7 artifacts: coverage metrics, deterministic 0..100 quality score with band caps (static-only ≤ 39, missing-bag ≤ 59, contradictions ≤ 39), gap detection + recommendations, operator review audit (explicit acknowledgement only), trends + cross-incident comparison + filterable index. | `rover_ws/tools/analyze_replay_coverage.py`, `rover_ws/tools/compare_replay_reviews.py`, `rover_ws/tools/generate_replay_analytics.py`, `rover_ws/tools/audit_replay_reviews.py` |
| [`reliability_impact`](../backend/app/reliability_impact/) | Phase 9. Read-only source-to-evidence traceability + analytics-delta gate. Classifies changed files by subsystem, maps subsystems to REQ-* ids + recommended tools/artifacts, compares replay analytics against a pinned baseline, assesses risk, and emits a deterministic CI gate decision. Missing live runtime evidence on github-hosted runners is **never** a failure. | `rover_ws/tools/analyze_source_impact.py`, `rover_ws/tools/reliability_impact_gate.py` |
| [`programme_review`](../backend/app/programme_review/) | Phase 10. Read-only longitudinal governance layer over Phase 3..9 artifacts. Deterministic trends, drift detection, six-discipline governance health rollup, subsystem-risk aggregation, coverage evolution, gate history, evidence freshness. Missing history is reported as `insufficient_history` / `unknown`, never as regression. | `rover_ws/tools/generate_programme_review.py`, `analyze_reliability_trends.py`, `detect_reliability_drift.py`, `review_governance_health.py`, `review_evidence_freshness.py` |
...
| [`live_runtime`](../backend/app/live_runtime/) | Phase 13. Live runtime evidence layer: bag manifest classifier, runner profile validator, scenario plan loader, evidence-capture honesty rules, maturity aggregator. Refuses to label static or missing evidence as `bag_backed`. | `rover_ws/tools/live_bag_capture.py`, `rover_ws/tools/validate_live_runtime_evidence.py`, `rover_ws/tools/process_live_runtime_evidence.py` |
| [`natural_language_mission`](../backend/app/natural_language_mission/) | Phase 14A. Deterministic, offline natural-language mission compiler: grammar templates, parser, ODD validator, risk classifier, audit + replay-binding artifacts. Never authorises motion. | `rover_ws/tools/compile_mission_intent.py`, `rover_ws/tools/validate_mission_plan.py`, `rover_ws/tools/generate_mission_audit.py` |
| [`mission_proposal`](../backend/app/mission_proposal/) | Phase 14B. Offline pluggable LLM-shaped seam: provider interface, deterministic mock provider, sanitizer chokepoint, adapter into Phase 14A, audit bundle. External LLM providers are intentionally disabled. | `rover_ws/tools/propose_mission_from_text.py`, `rover_ws/tools/validate_mission_proposal.py`, `rover_ws/tools/generate_mission_proposal_examples.py` |
| [`skill_authoring`](../backend/app/skill_authoring/) | Phase 15A. Offline deterministic robotics skill authoring workbench: bounded catalog, intent parser, code templates, validator, safety review, audit bundle with code-card metadata. No LLM, no execution. | `rover_ws/tools/generate_robotics_skill.py`, `rover_ws/tools/validate_robotics_skill.py`, `rover_ws/tools/generate_skill_examples.py` |
| [`skill_llm_provider`](../backend/app/skill_llm_provider/) | Phase 15B. Disabled-by-default local LLM skill candidate provider: fixture / disabled / local_http / ollama / llama_cpp modes, sanitizer chokepoint, validator bridge into Phase 15A. No cloud APIs, no network call, no code execution. | `rover_ws/tools/propose_robotics_skill_with_local_llm.py`, `rover_ws/tools/validate_skill_llm_candidate.py`, `rover_ws/tools/generate_skill_llm_examples.py` |
| [`mission_rehearsal`](../backend/app/mission_rehearsal/) | Phase 16. Governed mission-to-rehearsal pipeline: plan builder, validator, supervisor authority gate, deterministic state machine + event stream, replay bridge (never bag-backed), analytics bridge, audit bundle. Simulation-only; no hardware, no ROS, no network. | `rover_ws/tools/run_mission_rehearsal.py`, `rover_ws/tools/validate_mission_rehearsal.py`, `rover_ws/tools/generate_rehearsal_examples.py`, `rover_ws/tools/generate_rehearsal_replay.py` |
| [`mission-control`](../apps/mission-control/) | Phase 17A. Static-export Next.js operator console. Reads committed JSON artifacts; never opens a network socket. AST-level honesty tests forbid cloud SDK / network module imports. | `npm run build` produces 18 static pages; `npm run test` runs 26 vitest tests (no ROS, no network). |
| [`mission-control / spatial`](../apps/mission-control/src/adapters/spatial.ts) | Phase 17B. Deterministic 2D route adapter + spatial visualisation. Derives coordinates from bounded distance/angle inputs only; never reads telemetry; never fabricates positions. | Vitest suites `spatial.test.ts` and `deployment.test.ts`; the `mission-control-ci.yml` workflow runs `npm run typecheck && npm run test && npm run build` plus a prerendered-HTML honesty grep. |
| [`spatial_replay`](../backend/app/spatial_replay/) | Phase 17C. Bag-backed / fixture-derived spatial replay layer: manifest loader, pose extractor, trajectory builder, event aligner, validator, reporter. The `evaluate_bag_eligibility` + `is_honestly_bag_backed` pair is the single gatekeeper for the `bag_backed` label. Never parses real `.mcap` / `.db3` files; the operator post-processes the bag into a JSONL of pose samples. | Backend pytest `tests/test_spatial_replay.py`; frontend vitest `tests/spatial-replay.test.ts` + `tests/spatial-honesty.test.tsx`. CLI: `tools/generate_spatial_replay.py`. |
| [`artifact_registry`](../backend/app/artifact_registry/) | Phase 18. Canonical artifact-governance layer: discovery, deterministic sha256 hashing, lifecycle / integrity rules, hydration. The `evaluate_bag_eligibility` honesty rules from Phase 17C compose with `verify_artifact` here so a failing hydration never rewrites the registry. | Backend pytest `tests/test_artifact_registry.py` + `tests/test_replay_hydration.py`; frontend vitest `tests/replay-governance.test.ts` + `tests/artifact-lineage.test.tsx` + `tests/hydration-honesty.test.tsx`. CLI: `tools/hydrate_replay_artifacts.py`. CI: `.github/workflows/replay-hydration.yml`. |
| [`mission-control / 3d`](../apps/mission-control/src/3d/) | Phase 18. Immersive deterministic mission scene: React Three Fiber scene, deterministic camera rig per playback mode, WebGL-gated fallback to the Phase 17C 2D panel. The scene reads exclusively from committed replay artifacts; the honesty grep in `tests/hydration-honesty.test.tsx` rejects websocket / streaming / `setInterval` imports. | Frontend vitest `tests/3d-scene.test.tsx` + `tests/cinematic-playback.test.ts` + `tests/mission-storytelling.test.tsx` + `tests/artifact-lineage.test.tsx`. |
| [`mission-control / theme`](../apps/mission-control/src/styles/) | Phase 19. Token + theme system; light/dark toggle; no-FOUC bootstrap script. The honesty grep in `tests/design-tokens.test.ts` rejects pure-black / pure-white tokens, raw `#000` / `#fff` hex, and `bg-black` / `bg-white` Tailwind classes. | Frontend vitest `tests/theme.test.tsx` + `tests/design-tokens.test.ts` + `tests/responsive-layout.test.tsx`. |
| [`skill_llm_provider (Phase 19 intelligence)`](../backend/app/skill_llm_provider/) | Phase 19. Candidate normalizer + deterministic ranker + suggestion-only repair + readiness check + capability registry + safety critique. Validator outcome dominates ranking; the readiness check never opens a socket. | Backend pytest `tests/test_skill_llm_intelligence.py`. |
| [`scene_snapshot`](../backend/app/scene_snapshot/) | Phase 19. Reviewer scene-snapshot metadata pipeline + frontend `SceneSnapshotPanel`. A fixture run never becomes a bag-backed snapshot; the canonical fixture renders an honest `fixture` status. | Backend pytest `tests/test_scene_snapshot.py`; frontend vitest `tests/scene-snapshot.test.tsx`. CLI: `tools/generate_reviewer_scene_snapshot.py`. |
...
* canonical-fixture artifacts are committed and hashes match the
  registry.
