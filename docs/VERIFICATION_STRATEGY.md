...
| [`mission-control`](../apps/mission-control/) | Phase 17A. Static-export Next.js operator console. Reads committed JSON artefacts; never opens a network socket. AST-level honesty tests forbid cloud SDK / network module imports. | `npm run build` produces 18 static pages; `npm run test` runs 26 vitest tests (no ROS, no network). |
