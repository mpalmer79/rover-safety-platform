...
* No artifact in `skill-llm-candidates/` represents a real model
  inference.
* No artifact in `skill-llm-candidates/` represents a runtime
  mission or actuator authorisation.
* No artifact in `skill-llm-candidates/` is generated by a cloud
  LLM call.
